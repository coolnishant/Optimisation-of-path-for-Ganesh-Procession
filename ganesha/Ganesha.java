...
public class Ganesha {
    ...
    public static void main(String[] args) {
        ...
        int lakeat[] = {23,26,28};
        int nodeMatrix[][]={{1,3,4},{2,3,5},{3,2,6,4},{4,1,3,7,10},{5,2,6,8,12,11},{6,3,7,8,5},{7,4,6,9},{8,6,9,13,5},{9,7,8,10,14},{10,4,9,15},{11,5,12,16},{12,13,18,5,11},{13,12,8,14,19},{14,9,15,13},{15,10,14,22}, 
                            {16,11,17,23,24},{17,16,18,24},{18,17,12,19,25},{19,18,13,20},{20,19,21,26},{21,20,22,27},{22,21,29,15},{23,16,24},{24,23,16,17,25},{25,24,18,26},{26,25,20,27},
                            {27,21,29,26,28},{28,27,29},{29,28,22,27}};
        int weightMatrix[][]={{4,1},{4,2},{4,1,4},{1,4,1,4},{2,4,4,3,3},{1,4,1,4},{1,4,1},{1,4,3,4},{1,3,4,2},{4,3,1},{3,2,4},{1,1,3,2},{1,3,4,1},{2,3,4},{1,3,1},{4,2,3,3},{2,3,3},{3,1,1,3},{1,1,2},{2,2,4},{2,3,4},
                               {3,1,1},{3,2},{2,3,3,2},{2,3,2},{2,4,3},{4,4,3,3},{3,4},{4,1,4}};
        ...
        for(int j = 0;j<noofvertices;j++){
            ...
            for(int i = 0;i<lakeat.length;i++){
                ...
                if(cityMap.getFlag(j)==1){
                    System.out.println("Ganesha "+j+" distance with "+i+" is at"+allDist[i][j].distance);
                }
            }    
                if(cityMap.getFlag(j)==1){
                  ...
                  System.out.println("Ganesha "+j+" goes to "+minIsAt+" with dist "+allDist[minIsAt][j].distance);
                }
            }
        }
        for(int i = 0;i<lakeat.length;i++){
...
//        int smallClusters[] = new int[lakeat.length];
        ...
        for(int i = 0;i<lakeat.length;i++){
            if(eachClusterSize[i]>=balancingThres*min){
...
//                System.out.println("hello" + tempmin);
                ...
                for(i=0;i<eachClusterSize[maxisat];i++){
                   if(allDist[minisat][i].distance-((HeapNode)li.get(i)).distance<tempmin){
                       tempmin = allDist[minisat][i].distance-((HeapNode)li.get(i)).distance;
                       tempminat = i;
                   }
                }
                if(tempminat == -1)
...
//            while(lpt.getLast())
        ...
        for(int lk=0;lk<lakeat.length;lk++){
            ...
            System.out.println("Cluster Size ) "+eachClusterSize[lk]+" "+nowat);
