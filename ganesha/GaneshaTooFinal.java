...
//            for(int j=0;j<eachClusterSize[i];j++)
                System.out.println("Lake: "+lakeat[i]+" Conatains: "+eachClusterSize[i]);
        }
        System.out.println("");
        //TODO balancing
...
//                System.out.println("Heap Val "+((HeapNode)li.get(i)).distance);
                    if(allDist[minisat][i].distance<tempmin){
                       tempmin = ((HeapNode)li.get(i)).distance;
                       tempminat = i;
               }
            }
            if(tempminat == -1)
                break;
            System.out.println("\nMove: "+allDist[minisat][i].vertex+" from "+lakeat[maxisat]+" "+lakeat[minisat]);
...
//                System.out.println("Edge: "+ distForAll[minGanesha[0]][minGanesha[1]].vertex);
